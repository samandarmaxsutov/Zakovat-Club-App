package uz.it_school.zakovatclubapp.utils
...
abstract class SharedPreferenceHelper(context: Context, preferences: SharedPreferences? = null) {
    ...
    inner class ints(private val defValue: Int = -1) : ReadWriteProperty<Any, Int> {
        override fun getValue(thisRef: Any, property: KProperty<*>) = pref.getInt(property.name+"1", defValue)
        override fun setValue(thisRef: Any, property: KProperty<*>, value: Int) = pref.edit { putInt(property.name+"1", value).apply() }
    }
